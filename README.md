...
- 当前阶段：已实现规则优先 + LLM 兜底意图分类、意图路由、hybrid + pure_kb 融合；默认 `USE_ROUTER=true`，后续重点是加权融合、正式评测和真实环境归档
...
| 调试输出 | `python main.py --debug "..."`，输出意图识别、路由、两路检索、融合、生成和校验摘要 |
...
| 目录/文件 | 内容 |
|---|---|
| `main.py` | 当前统一入口：意图路由、hybrid + pure_kb、生成、引用校验、`--debug` |
| `vector_kb/` | 向量库、BM25、RRF、混合检索、生成、引用校验、意图分类和领域过滤模块 |
| `pure_kb/` | 独立纯知识库后端：六领域分区、显式 domains/filters 检索、精确元数据查询；已接入检索路由 |
...
| `data/evaluation/` | 50 条测试用例题集及字段说明 |
...
- 纯知识库已落位到 `pure_kb/`，198 条、六领域、坏向量 0，并通过 `route_and_retrieve()` 接入主链路。
...
回归覆盖领域过滤、纯向量、BM25、混合检索、RRF、引用校验、主链路重试、调试输出和数据一致性。另已完成 7 条真实 DeepSeek A/B 对比：A 路 Top-5 命中率 50.00%，B 路 83.33%。

...
## 附三、主干阶段贡献归属（2026-09-14）

> 记录口径：按「谁负责 / 产出什么 / 评审与接续」描述，便于仓库交接与考核核验；只写事实与产出，不作评价性表述。

### 队长
- **口径与规则**：确定技术向、油浸式电力变压器、DL/T 722-2014 口径（AGENTS.md）；IEC 60599 因费用弃用的说明。
- **工程主线**：Python 环境与依赖（venv / requirements.txt）；Git 仓库、推送与提交规范、协作者接入、阶段交接约定。
- **标准与语料**：DL/T 722-2014 真本核验；DL/T 572-2021 正文还原与 §7.5/第 8 章 OCR 补读；判据表结构化导出；DGA 样本 3466 条下载与量纲统一；《切条与元数据规范》（docs/07）。
- **接口与评测**：`retrieve()` 可复用接口与 `page` 打通（P0）；`generate()` 接入 DeepSeek（强制引用 + 拒答 + 错误码提示）；`ask` 子命令；规则基线评测 61.8%（docs/04）。
- **文档与治理**：docs 合并精简、路径英文化、版权规避、验收记录（docs/08）。

### 成员2
- **领域与判据**：人工核对 DL/T 722-2014 表6/表7（Excel），支撑规则库升级至 v0.3（VERIFIED）。
- **知识库建设（承接）**：完成语料范围收敛（保留 722/572 两份标准）、补齐 `clause` 元数据、采用 bge-m3 1024 维向量，产出第三版知识库；为后续检索与生成提供了可用基础。
- **配套完善**：与队长配合完成入库脚本、输入 JSONL、WAL 清理与相对路径改造，使知识库达到可复现要求并并入 `vector_kb/`。
- **后续分工**：阶段二语料（572 表格结构化、GB 26860 摘录、中文故障案例）。

### 成员1
- **探索与初版**：率先完成向量知识库的初步搭建与两次提交，打通了「文档入库 → 向量检索」的雏形，为团队确定技术方向提供了可对比的实现。
- **进展说明**：经内部评审，初版在「条号元数据、分块粒度、可复现性」等方面**仍有提升空间**，未完全达到当前评审口径；为避免在同一结构上反复返工，经沟通后将知识库完善工作**移交成员2接续推进**。
- **沉淀价值**：这两轮探索让团队明确了「检索结果必须带条号」「切块要按条款而非固定长度」等关键约束，相关结论已写入 docs/07，成为后续开发的规范依据。

### 成员3
- **文献精读**：完成 C2/B1/C1/C3/C4 等核心论文阅读，产出精读卡、略读卡与读论文笔记（docs/notes/）。
- **待完成**：任务一综述初稿、检索与引用评测（作为下一阶段的主要内容）。

---

> 备注：以上按主干阶段（09-12 → 09-26）记录；如有遗漏或表述需要调整，可在下次对表时补充，并同步更新本节与 docs/00。
